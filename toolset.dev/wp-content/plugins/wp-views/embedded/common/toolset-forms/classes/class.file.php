...
/**
 *
 * $HeadURL: https://www.onthegosystems.com/misc_svn/common/tags/Types1.6b4-CRED1.3b4-Views1.6.2b2/toolset-forms/classes/class.file.php $
...
class WPToolset_Field_File extends WPToolset_Field_Textfield
{
    ...
    public static function file_enqueue_scripts() {
        ...
        if ( !wp_script_is( 'wptoolset-field-file', 'enqueued' ) ) {
			...
			$js_data = array('title' => __( 'Select file', 'wpv-views' ), 'for_post' => $for_post, 'adminurl' => admin_url());
			wp_localize_script( 'wptoolset-field-file', 'wptFileData', $js_data );
		}
		if ( !wp_style_is( 'wptoolset-field-file' ) ) {
    ...
    public function metaform() {
        ...
        // Set button
        $button = sprintf(
            '<a href="#" class="js-wpt-file-upload button-secondary" data-wpt-type="%s">%s</a>',
            $this->getType(),
            sprintf( __( 'Select %s', 'wpv-views' ), $this->getType() )
        );
