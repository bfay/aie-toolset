...
class WPToolset_Field_Date extends FieldFactory
{
    ...
    public function metaform() {
		...
		$attr_visible = array('class' => $def_class, 'style' => 'width:150px;', 'readonly' => 'readonly');
		...
		*/
        if ( !empty( $data['add_time'] ) ) {
			...
			if ( $readonly ) {
				$hour_element['#attributes'] = array( 'disabled' => 'disabled' );
			}
            $form[] = $hour_element;
            // Minutes
            $minutes = 60;
			...
			if ( $readonly ) {
				$minute_element['#attributes'] = array( 'disabled' => 'disabled' );
			}
            $form[] = $minute_element;
        }
