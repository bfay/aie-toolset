...
function optionsframework_tabs() {
	...
	foreach ($options as $value) {
		...
		if ($value['type'] == "heading") {
			...
			$menu .= '<a id="'.  esc_attr( $jquery_click_hook ) . '-tab" class="nav-tab" title="' . esc_attr( $value['name'] ) . '" href="' . esc_attr( '#'.  $jquery_click_hook ) . '">' . esc_html( $value['translation'] ) . '</a>';
		}
	}
...
function optionsframework_fields() {
	...
	foreach ( $options as $value ) {
		...
		case "heading":
			...
			$output .= '<h3>' . esc_html( $value['translation'] ) . '</h3>' . "\n";
			break;
		}
